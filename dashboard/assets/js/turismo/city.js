document.addEventListener('DOMContentLoaded', function(){
  ...
  //Selecionar itens
  $(document).delegate('#city-id', 'click', function() {
    if (this.checked) {
  ...
  if (document.querySelector('#modal-delete')){
    ...
    _seleted = 0;
    document.querySelectorAll('.table input[city-select]').forEach(function(element, index){
      if (element.checked) {
        _seleted++;
      }
    });
    ...
    if(_seleted > 0){
      document.querySelector('#remove-content').innerHTML = _seleted+' cidade/província(s) selecionada(s) pretende mesmo removela(s)?';
      document.getElementById('remove-item').hidden       = false;
    }else{
      document.querySelector('#remove-content').innerHTML = 'Nenhuma cidade/província selecionada!';
  ...
  document.getElementById('remove-item').addEventListener('click',function(){
    ...
    document.querySelectorAll('.table input[city-select]').forEach(function(element, index){
      if (element.checked) {
        _seleted += element.getAttribute('city-select')+',';
      }
    });
