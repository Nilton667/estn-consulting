document.addEventListener('DOMContentLoaded', function(){
    ...
    //Selecionar itens
    $(document).delegate('#cursos-id', 'click', function() {
      if (this.checked) {
    ...
    if (document.querySelector('#modal-delete')){
      ...
      _seleted = 0;
      document.querySelectorAll('.table input[cursos-select]').forEach(function(element, index){
        if (element.checked) {
          _seleted++;
        }
      });
      ...
      if(_seleted > 0){
        ...
        document.querySelector('#remove-content').innerHTML = 'Nenhuma curso selecionado!';
    ...
    document.getElementById('remove-item').addEventListener('click',function(){
      ...
      document.querySelectorAll('.table input[cursos-select]').forEach(function(element, index){
        if (element.checked) {
          _seleted += element.getAttribute('cursos-select')+',';
        }
      });
