document.addEventListener('DOMContentLoaded', function(){
  ...
  //Selecionar itens
  $(document).delegate('#imoveis-id', 'click', function() {
    if (this.checked) {
  ...
  if (document.querySelector('#modal-delete')){
    ...
    _seleted = 0;
    document.querySelectorAll('.table input[imoveis-select]').forEach(function(element, index){
      if (element.checked) {
        _seleted++;
      }
    });
  ...
  document.getElementById('remove-item').addEventListener('click',function(){
    ...
    document.querySelectorAll('.table input[imoveis-select]').forEach(function(element, index){
      if (element.checked) {
        _seleted += element.getAttribute('imoveis-select')+',';
      }
    });
